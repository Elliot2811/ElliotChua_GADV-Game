...
{   // This script handles forces applied on skateboard such as movement
    ...
    public void SkateboardSpriteDirection()
    {

        if (m_boardRb != null && m_spriteRenderer != null)
        {
            float velocityX = m_boardRb.velocity.x;

            if (velocityX > 1 || velocityX < -1) // Need some velocity before flipping the sprite
            {
                m_spriteRenderer.flipX = m_boardRb.velocity.x < 0; // Flip the skateboard sprite based on velocity direction
            }
        }
    }
